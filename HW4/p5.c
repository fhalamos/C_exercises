...
{
	int * new_array = (int*) malloc(MAX_WORD_LENGTH*sizeof(int));
	for (int i = 0; i < MAX_WORD_LENGTH; ++i)
		new_array[i] = a[i];
	return new_array;
}

void set_array_a(Hashmap* dictionary,int * best_array)
{
	for (int i = 0; i < MAX_WORD_LENGTH; ++i)
		dictionary->a[i] = best_array[i];
}

void optimize(Hashmap* dictionary)
{
	int max_entries = calculate_max_entries(dictionary);
	int local_max_entries;
	...
	set_array_a(dictionary,best_array);
	rehash_all_elements(dictionary);
}
...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		/* Optimize*/
		if( strncmp(command, "optimize", COMMAND_LEN) == 0 )
			optimize(dictionary);
