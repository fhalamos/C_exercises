...
typedef struct hashmap{
	...
	//used to generate vector a of random integer values, which defines each specific hash function
    unsigned int seed;
...
unsigned int naive_hash(char * word, int nbins, int seed)
{
	unsigned int h = 0;
	int c;
...
unsigned int bernstein_hash(char * word, int nbins, int seed)
{
	unsigned int h = 5381;
	int c;
	while(c = *word++)
...
unsigned int FNV_hash(char * word, int nbins, int seed)
{
...
unsigned int universal_hash(char * word, int n_bins, int seed)
{
	...
	int * a = (int*) malloc(MAX_WORD_LENGTH*sizeof(int));
...
{
	unsigned int index = (*(dictionary->hash_algorithm))(word, dictionary->n_bins, dictionary->seed); 
